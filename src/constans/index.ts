...
export const getCenter = (id: number) => {
  return centers.find((center) => center.id === id);
};

export const getIVAC = (id: number) => {
  return ivacs.find((ivac) => ivac.id === id);
};

export const getVisaType = (id: number) => {
  return visaTypes.find((visaType) => visaType.id === id);
};

export const getPaymentMethod = (slug: string) => {
